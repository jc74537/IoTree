...
func main() {
	...
	soundPtr := flag.String("s", "/dev/ttyACM0", "Path to tty of sound arduino")
	flag.Parse()
	port = *portPtr
	sound.Setup(*soundPtr)
...
func lightHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || idNo >= len(patterns) {
		fmt.Println("Failed to run pattern: " + id)
...
func songHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || idNo >= len(library) {
		fmt.Println("Failed to play song: " + id)
		w.WriteHeader(http.StatusNotFound)
	} else {
